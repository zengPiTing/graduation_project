...
exports.lodin_handler = (req, res) => {
    ...
    const sql_student = 'select * from user_students where student_number=? and status=0 and isManager=0';
    const sql_teacher = 'select * from user_teacher where username=? and status=0 and isManager=0';
    //查询学生用户中是否有此用户
...
//用户修改密码路由模块回调函数
exports.updatePassword_handler = (req, res) => {
    ...
    if (isTeacher == '0') { //老师用户
        db.query(sql_old_teacher, id, (err, result) => {
            ...
            if (result.length !== 1) return res.cc('修改失败，稍后再试');
